...
# =========================================================
# PROJECT MADDEN EA DIRECT CONNECTOR (BETA)
# =========================================================

EA_CONNECTIONS_FILE = "ea_connections.json"

# Madden 27 Companion App compatibility values. These mirror the current
# Companion App flow used by open-source Madden dashboard tooling.
EA_MADDEN_YEAR = os.environ.get(
    "EA_MADDEN_YEAR",
    "2027"
).strip()

EA_MADDEN_TWO_DIGIT_YEAR = os.environ.get(
    "EA_MADDEN_TWO_DIGIT_YEAR",
    "27"
).strip()

EA_CLIENT_ID = os.environ.get(
    "EA_MADDEN_CLIENT_ID",
    "MCA_27_COMP_APP"
).strip()

EA_CLIENT_SECRET = os.environ.get(
    "EA_MADDEN_CLIENT_SECRET",
    ""
).strip()

EA_AUTH_SOURCE = os.environ.get(
    "EA_MADDEN_AUTH_SOURCE",
    "317239"
).strip()

EA_MACHINE_KEY = os.environ.get(
    "EA_MADDEN_MACHINE_KEY",
    "444d362e8e067fe2"
).strip()

EA_REDIRECT_URL = os.environ.get(
    "EA_MADDEN_REDIRECT_URL",
...
EA_LOGIN_URL = (
    ...
    "&release_type=prod"
    "&response_type=code"
    f"&redirect_uri={EA_REDIRECT_URL}"
    f"&client_id={EA_CLIENT_ID}"
    f"&machineProfileKey={EA_MACHINE_KEY}"
    f"&authentication_source={EA_AUTH_SOURCE}"
)

EA_BLAZE_BASE = os.environ.get(
    "EA_BLAZE_BASE",
    "https://wal2.tools.gos.bio-iad.ea.com/wal"
).strip().rstrip("/")

EA_ENTITLEMENTS = {
    f"MADDEN_{EA_MADDEN_TWO_DIGIT_YEAR}XONE": "xone",
    f"MADDEN_{EA_MADDEN_TWO_DIGIT_YEAR}PS4": "ps4",
    f"MADDEN_{EA_MADDEN_TWO_DIGIT_YEAR}PC": "pc",
    f"MADDEN_{EA_MADDEN_TWO_DIGIT_YEAR}PS5": "ps5",
    f"MADDEN_{EA_MADDEN_TWO_DIGIT_YEAR}XBSX": "xbsx",
    f"MADDEN_{EA_MADDEN_TWO_DIGIT_YEAR}SDA": "stadia",
}
...
EA_BLAZE_SERVICE = {
    "xone": f"madden-{EA_MADDEN_YEAR}-xone",
    "ps4": f"madden-{EA_MADDEN_YEAR}-ps4",
    "pc": f"madden-{EA_MADDEN_YEAR}-pc",
    "ps5": f"madden-{EA_MADDEN_YEAR}-ps5",
    "xbsx": f"madden-{EA_MADDEN_YEAR}-xbsx",
    "stadia": f"madden-{EA_MADDEN_YEAR}-stadia",
}

EA_BLAZE_PRODUCT = {
    "xone": f"madden-{EA_MADDEN_YEAR}-xone-mca",
    "ps4": f"madden-{EA_MADDEN_YEAR}-ps4-mca",
    "pc": f"madden-{EA_MADDEN_YEAR}-pc-mca",
    "ps5": f"madden-{EA_MADDEN_YEAR}-ps5-mca",
    "xbsx": f"madden-{EA_MADDEN_YEAR}-xbsx-mca",
    "stadia": f"madden-{EA_MADDEN_YEAR}-stadia-mca",
}
...
def ea_connector_configured():
    return bool(
        EA_CLIENT_ID
        and EA_CLIENT_SECRET
        and EA_AUTH_SOURCE
        and EA_MACHINE_KEY
    )


def ea_encryption_key():
    explicit = os.environ.get(
        "PROJECT_MADDEN_EA_ENCRYPTION_KEY",
        ""
    ).strip()
    ...
    seed = (
        ...
        or discord_public_key()
        or "project-madden-ea-connector"
    )
...
def ea_store_pending(
    setup_token,
    payload
):
    data = load_ea_connections()
    ...
    return {
        ...
        "User-Agent":
            (
                ...
                "sdk_gphone_x86_64 Build/TE1A.220922.031)"
            ),
        "Content-Type":
...
def ea_exchange_authorization_code(
    code_value
):
    if not ea_connector_configured():
        raise RuntimeError(
            "EA direct connector is not configured. "
            "Set EA_MADDEN_CLIENT_SECRET in Render."
        )

    code_value = ea_extract_code(
        code_value
    )

    if not code_value:
        raise RuntimeError(
            "No EA authorization code was found."
        )

    response = requests.post(
        "https://accounts.ea.com/connect/token",
        headers=ea_form_headers(),
        data={
    ...
    if not payload.get(
        ...
        raise RuntimeError(
            "EA did not return an access token."
        )
...
def ea_tokeninfo(
    ...
    response = requests.get(
        (
            "https://accounts.ea.com/connect/tokeninfo"
    ...
    if not response.ok:
        raise RuntimeError(
            "EA token information request failed: "
            + response.text[:500]
        )
...
def ea_entitlements(
    ...
    response = requests.get(
        (
            "https://gateway.ea.com/proxy/identity/"
            f"pids/{pid}/entitlements/?status=ACTIVE"
        ),
        headers={
            "User-Agent":
                ea_form_headers()[
    ...
    if not response.ok:
        raise RuntimeError(
            "EA Madden entitlement request failed: "
            + response.text[:500]
        )
...
def ea_personas_for_entitlement(
    access_token,
    entitlement
):
    pid_uri = entitlement.get(
        "pidUri"
    )
    ...
    if not response.ok:
        raise RuntimeError(
            "EA Madden persona request failed: "
            + response.text[:500]
        )
...
def ea_discover_personas(
    ...
    if not pid:
        raise RuntimeError(
            "EA account PID was not returned."
        )

    entitlement_payload = ea_entitlements(
        access_token,
        pid
    )
    ...
    if not valid:
        raise RuntimeError(
            "This EA account does not show an active Madden 27 "
            "online entitlement."
        )

    final = []

    for entitlement in valid:
        console = EA_ENTITLEMENTS[
            entitlement[
                "groupName"
            ]
        ]

        personas = ea_personas_for_entitlement(
            access_token,
            entitlement
        )
    ...
    if not final:
        raise RuntimeError(
            "No Madden persona was found for this EA account."
        )

    return final


def ea_persona_token(
    first_access_token,
    persona
):
    url = (
        ...
        f"&machineProfileKey={EA_MACHINE_KEY}"
        f"&authentication_source={EA_AUTH_SOURCE}"
        f"&access_token={first_access_token}"
        f"&persona_id={persona['personaId']}"
        f"&persona_namespace={persona['namespaceName']}"
    )
        ...
        headers={
            ...
            "User-Agent":
                (
                    ...
                    "sdk_gphone_x86_64 Build/TE1A.220922.031; wv) "
                    "AppleWebKit/537.36 (KHTML, like Gecko) "
                    "Version/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36"
                ),
            "Accept":
                (
    ...
    if not location:
        raise RuntimeError(
            "EA did not return the Madden persona authorization redirect."
        )
    ...
    if not code_value:
        raise RuntimeError(
            "EA Madden persona authorization code was not found."
        )
    ...
    if not response2.ok:
        raise RuntimeError(
            "EA Madden persona token exchange failed: "
            + response2.text[:500]
        )
...
def ea_retrieve_blaze_session(
    ...
    try:
        ...
        raise RuntimeError(
            "EA Blaze session response was incomplete: "
            + text[:500]
        )
...
def ea_message_auth(
    ...
    static_bytes = bytes.fromhex(
        "634203362017bf72f70ba900c0aa4e6b"
    )

    xor_hash = hashlib.md5(
        random_bytes
        + static_bytes
    ).digest()
    ...
    static_auth_code = bytes.fromhex(
...
def ea_send_blaze_request(
    ...
    try:
        ...
        raise RuntimeError(
            "EA Blaze request returned invalid JSON: "
            + text[:500]
        )
...
def ea_get_leagues(
    ...
    token = ea_refresh_token(
        token
    )

    if session is None:
        session = ea_retrieve_blaze_session(
            token
        )

    payload = ea_send_blaze_request(
        token,
        session,
        "Mobile_GetMyLeagues",
        2060,
...
def ea_get_export_data(
    ...
    try:
        ...
        raise RuntimeError(
            "EA export returned invalid JSON: "
            + cleaned[:500]
        )
...
def ea_direct_sync_core(
    guild_id
):
    connection = ea_load_connection(
        guild_id
    )

    if not connection:
        raise RuntimeError(
            "No direct EA connection is saved for this server."
        )
    ...
    league_info = ea_get_league_info(
        token,
        session,
        league_id
    )

    teams = ea_get_export_data(
        token,
        session,
        EA_EXPORT_TYPES[
            "teams"
    ...
    # Save into the current Project Madden data shape so the existing
    # dashboard/analyst code can immediately use the direct EA source.
    save_json_file(
        "leagueteams.json",
        teams
    )
...
<div class="wrap">
  ...
  <div class="panel">
    ...
    <div class="info"><b>Important:</b> Project Madden now includes an EA Direct Connector beta. Snallabot can still be used as a fallback while we validate the direct EA connection across leagues and platforms.</div>
    ...
    <div class="auto">
      <div class="auto-head">
        <div>
          <div class="auto-title">🎮 EA Direct Connector <span style="color:#55b8ff">BETA</span></div>
          <div class="note">Connect Project Madden directly to your EA/Madden franchise. Snallabot remains available as a fallback while this beta is being validated.</div>
        </div>
        <span class="badge wait">BETA</span>
      </div>
      <a href="/dashboard/ea/{{ guild.setup_token }}" style="display:block;text-align:center;margin-top:14px;padding:14px;border-radius:12px;background:linear-gradient(135deg,#58baff,#7c62ff);color:#05111c;font-weight:950;text-decoration:none">CONNECT WITH EA</a>
    </div>
    ...
    <form method="post">
      ...
      <div class="note">Optional if you connect through EA Direct. Keep it if you want Snallabot as a fallback.</div>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>EA Direct Connect • Project Madden</title>
<style>
...
h1{margin-top:0;font-size:36px}.muted{color:var(--muted);line-height:1.6}.step{background:#0b111a;border:1px solid var(--line);border-radius:14px;padding:16px;margin:14px 0}
.btn,button{display:inline-block;background:var(--accent);color:#06111c;border:0;border-radius:11px;padding:13px 16px;font-weight:900;text-decoration:none}
input,select{width:100%;padding:13px 14px;background:#080f17;color:var(--text);border:1px solid var(--line);border-radius:10px;font-size:16px}
label{display:block;font-weight:850;margin:16px 0 7px}.ok{background:#10291d;border:1px solid #245b3e;color:#85eeb2;padding:12px;border-radius:10px}
.warn{background:#302912;border:1px solid #5a4b1c;color:#f1d071;padding:12px;border-radius:10px}.footer{text-align:center;color:var(--muted);padding-top:24px;font-size:13px}
</style>
</head>
<body>
<div class="wrap">
<div class="panel">
<h1>EA Direct Connector <span style="color:#55b8ff">BETA</span></h1>
<p class="muted">Server: <b>{{ guild.guild_name or guild.guild_id }}</b></p>

{% if not configured %}
<div class="warn"><b>Connector setup required.</b> Add <code>EA_MADDEN_CLIENT_SECRET</code> in Render before using EA Direct Connect.</div>
{% else %}
<div class="ok">EA connector backend is configured.</div>
{% endif %}

<div class="step">
<h3>1. Sign in with EA</h3>
<p class="muted">Tap the EA login button. After EA signs you in, the browser may try to open <b>127.0.0.1</b> and show an error. That is expected. Copy the full URL from the address bar and paste it below.</p>
<a class="btn" href="{{ ea_login_url }}" target="_blank" rel="noopener">OPEN EA LOGIN</a>

<form method="post" action="/dashboard/ea/personas/{{ guild.setup_token }}">
<label>Paste the EA redirect URL</label>
<input name="ea_redirect_url" placeholder="http://127.0.0.1/success?code=..." required>
<button type="submit" style="margin-top:14px">CONTINUE</button>
</form>
</div>

{% if pending and pending.personas %}
<div class="step">
<h3>2. Choose your Madden account</h3>
<form method="post" action="/dashboard/ea/leagues/{{ guild.setup_token }}">
<label>Madden Persona</label>
<select name="persona_index">
{% for persona in pending.personas %}
<option value="{{ loop.index0 }}">{{ persona.displayName }} • {{ persona.console|upper }}</option>
{% endfor %}
</select>
<button type="submit" style="margin-top:14px">FIND MY LEAGUES</button>
</form>
</div>
{% endif %}

{% if pending and pending.leagues %}
<div class="step">
<h3>3. Select your Franchise</h3>
<form method="post" action="/dashboard/ea/connect/{{ guild.setup_token }}">
<label>Madden League</label>
<select name="league_id">
{% for league in pending.leagues %}
<option value="{{ league.leagueId }}">{{ league.leagueName }}{% if league.userTeamName %} • {{ league.userTeamName }}{% endif %}</option>
{% endfor %}
</select>
<button type="submit" style="margin-top:14px">CONNECT LEAGUE</button>
</form>
</div>
{% endif %}
...
{% if connection %}
<div class="step">
<h3>Direct EA Connection</h3>
<div class="ok">Connected to <b>{{ connection.league_name }}</b> ({{ connection.league_id }}) on {{ connection.token.console|upper }}.</div>
<form method="post" action="/dashboard/ea/sync/{{ guild.setup_token }}">
<button type="submit" style="margin-top:14px">SYNC FROM EA NOW</button>
</form>
</div>
{% endif %}
...
@app.route(
    "/dashboard/ea/<setup_token>"
)
def project_madden_ea_connector(
    setup_token
):
    guild = get_guild_config_by_token(
        setup_token
    ...
    return render_template_string(
        EA_CONNECTOR_HTML,
        guild=guild,
        configured=ea_connector_configured(),
        ea_login_url=EA_LOGIN_URL,
        pending=pending,
...
@app.route(
    "/dashboard/ea/personas/<setup_token>",
    methods=[
        "POST"
    ]
)
def project_madden_ea_personas(
    setup_token
):
    guild = get_guild_config_by_token(
        setup_token
    ...
    try:
        token = ea_exchange_authorization_code(
            request.form.get(
                "ea_redirect_url",
                ""
            )
        ...
        return redirect(
            f"/dashboard/ea/{setup_token}",
            code=302
        )

    except Exception as e:
        return (
            "<!doctype html><body style='background:#070b12;color:white;"
            "font-family:system-ui;padding:24px'><h1>EA Connection Error</h1>"
            f"<p>{str(e)[:1500]}</p>"
            f"<p><a style='color:#55b8ff' href='/dashboard/ea/{setup_token}'>Try again</a></p>"
            "</body>",
            400
        )
...
@app.route(
    "/dashboard/ea/leagues/<setup_token>",
    methods=[
        "POST"
    ]
)
def project_madden_ea_leagues(
    setup_token
):
    guild = get_guild_config_by_token(
        setup_token
    ...
    if not pending:
        return (
            "EA login session expired. Start again.",
            400
        )
    ...
    try:
        ...
        leagues, final_token, session = ea_get_leagues(
            final_token
        )

        if not leagues:
            raise RuntimeError(
                "EA returned no connected Madden leagues for this persona."
            )
        ...
        return (
            "<!doctype html><body style='background:#070b12;color:white;"
            "font-family:system-ui;padding:24px'><h1>EA League Discovery Error</h1>"
            f"<p>{str(e)[:1500]}</p>"
            f"<p><a style='color:#55b8ff' href='/dashboard/ea/{setup_token}'>Back</a></p>"
            "</body>",
            400
        )
...
@app.route(
    "/dashboard/ea/connect/<setup_token>",
    methods=[
        "POST"
    ]
)
def project_madden_ea_connect(
    setup_token
):
    guild = get_guild_config_by_token(
        setup_token
    ...
    if not selected:
        return (
            "Invalid Madden league selection.",
            400
        )
...
@app.route(
    "/dashboard/ea/sync/<setup_token>",
    methods=[
        "POST"
    ]
)
def project_madden_ea_sync(
    setup_token
):
    guild = get_guild_config_by_token(
        setup_token
...
@app.route(
    "/dashboard/ea/status/<setup_token>"
)
def project_madden_ea_status(
    setup_token
):
    guild = get_guild_config_by_token(
        setup_token
